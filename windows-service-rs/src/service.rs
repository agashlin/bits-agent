...
bitflags! {
    ...
    pub struct ServiceAccess: u32 {
        ...
        /// Can enumerate dependent services
        const ENUMERATE_DEPENDENTS = winsvc::SERVICE_ENUMERATE_DEPENDENTS;

        /// Can query the service configuration
        const QUERY_CONFIG = winsvc::SERVICE_QUERY_STATUS;

        /// Can specify a user-defned control code
        const USER_DEFINED_CONTROL = winsvc::SERVICE_USER_DEFINED_CONTROL;

        /// Can query and set the SACL
        const ACCESS_SYSTEM_SECURITY = winnt::ACCESS_SYSTEM_SECURITY;

        /// Can query the security descriptor
        const READ_CONTROL = winnt::READ_CONTROL;

        /// Can set the DACL
        const WRITE_DAC = winnt::WRITE_DAC;

        /// Can modify the owner or group
...
impl Service {
    ...
    pub fn start(&self) -> Result<()> {
        let rc = unsafe { winsvc::StartServiceW(self.service_handle.raw_handle(), 0, ptr::null_mut()) };
        if rc == 0 {
